...
function expandMe(val) {
    if (collapse_open.value === val) {
        collapse_open.value = 0;
    } else {
        collapse_open.value = val;
        if (val === 1) {
        ...
        <div class="flex flex-col mt-2 border-green-300 border-t dark:border-gray-700">
            ...
            <!-- <div :class="`text-white font-semibold hover:bg-blue-500 dark:hover:bg-gray-700 ${$route.path.includes('Navigation')? 'bg-blue-700 dark:bg-gray-700' : ''} ${collapse_open.value === 1? 'bg-blue-700 dark:bg-gray-700' : ''}`">
                <div class="px-5 py-3 cursor-pointer block" @click="expandMe(1)">
                    <span class="text-end text-white">{{collapse_open.value === 1? '▾' : '▸'}}</span> Management
                </div>

                <div class="flex flex-col pl-5" :class="{
                        'overflow-hidden transition-all duration-300': true,
                        'max-h-0': collapse_open.value !== 1,
                        'max-height-px mt-4': collapse_open.value === 1
                    }">
